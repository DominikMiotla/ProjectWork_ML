...
if __name__ == "__main__":
    ...
    print("\nShape di X_train:", X_train.shape)
    ...
    print("Applico SMOTE sulla classe 0 per bilanciare...")

    smote = SMOTE(sampling_strategy={0: y_train.value_counts()[1]}, random_state=42)
    X_train_aug, y_train_aug = smote.fit_resample(train_embeddings, y_train)
    
    X_train_aug = np.asarray(X_train_aug)
    y_train_aug_np = np.asarray(y_train_aug)
    
    y_train_aug_series = pd.Series(y_train_aug_np)
    stampa_distribuzione(y_train_aug_series, "training set (dopo SMOTE)")

    # Plot embeddings dopo SMOTE
    plot_embeddings(X_train_aug, y_train_aug_series, title="t-SNE embeddings dopo SMOTE", save_path="output/tsne_embeddings_train_SMOTE.png")

    # --------------------------
    # Addestramento con PivotTree
    # --------------------------
    ...
    for max_depth in max_depth_values:
        for min_samples_leaf in min_samples_leaf_values:
            ...
            pt.fit(X_train_aug, y_train_aug_np)
